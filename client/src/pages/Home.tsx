...
function quizAnswersMatch(answer: string, expected: string, prompt: "english" | "meaning") {
  ...
  if (normalizedAnswer === normalizedExpected || normalizedExpected.includes(normalizedAnswer) || normalizedAnswer.includes(normalizedExpected)) return true;
  if (prompt === "meaning") return meaningMatches(expected, answer);
  const distance = levenshtein(normalizedAnswer, normalizedExpected);
  return distance <= Math.max(1, Math.floor(normalizedExpected.length / 4));
}
...
export default function Home() {
  ...
  const submitQuizAnswer = () => {
    if (!quizCard) return;
    if (!quizAnswer.trim()) {
      toast.error("請先輸入你的答案。");
      return;
    }
    const expected = quizCard.prompt === "english" ? quizCard.entry.meaning : quizCard.entry.text;
    const correct = quizAnswersMatch(quizAnswer, expected, quizCard.prompt);
    ...
    if (!correct) {
      ...
      toast("答案需要再複習", { description: "已加入本次錯題，下面會顯示英文與中文意思。" });
    } else {
      toast.success("答對了！", { description: "你的答案與收藏內容相同或意思相近。" });
    }
  };
